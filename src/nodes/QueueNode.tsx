...
export const QueueNode = memo(function QueueNode({ data, selected }: NodeProps) {
  const status = String(data['status'] ?? 'default');
  const badges = (data['badges'] as string[] | undefined) ?? [];

  return (
    <div className={`af-node ${selected ? 'selected' : ''}`} role="button" tabIndex={0} aria-label={`Queue: ${String(data['label'])}`}>
